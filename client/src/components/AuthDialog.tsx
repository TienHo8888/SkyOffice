...
export default function AuthDialog() {
  ...
  return <div className="auth-screen"><div className="auth-card"><div className="auth-brand"><span className="studio-brand-mark"><i /><i /><i /></span><div><strong>TOHI<span>/</span>STUDIO</strong><small>GAME STUDIO OS</small></div></div><span className="auth-kicker">PRIVATE STUDIO WORKSPACE</span><h1>Build better games,<br /><em>together.</em></h1><p className="auth-intro">Vào game thôi — cùng team ship task, hạ sprint boss và gom Coin nhé ✦</p><form onSubmit={handleSubmit}><label>Tài khoản<input autoFocus autoComplete="username" placeholder="Username hoặc email" type="text" value={identifier} onChange={(event) => setIdentifier(event.target.value)} /></label><label>Mật khẩu<input autoComplete="current-password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} /></label>{error && <div className="auth-error">{error}</div>}<button className="auth-submit" disabled={loading} type="submit">{loading ? 'Connecting…' : 'Enter the studio →'}</button></form></div></div>
}
