...
export default function PlayerHud() {
  ...
  const openHome = () => {
    ...
    void network.joinHome(ownerId).catch(() => undefined)
  }
  ...
  return <aside className="player-hud" aria-label="Thông tin người chơi">
    ...
    <div className="player-hud-content">
      <div className="player-hud-head">
        ...
        <button className="player-hud-logout" type="button" title="Đăng xuất khỏi tài khoản" aria-label="Đăng xuất khỏi tài khoản" aria-keyshortcuts="Control+Shift+L Meta+Shift+L" onClick={() => dispatch(clearAuthSession())}>↪</button>
      </div>
      <div className="player-hud-stats"><span className="player-hud-xp">{levelXp.toLocaleString()} / {levelXpTarget.toLocaleString()} EXP</span></div>
      <div className="player-hud-progress" aria-label={`${levelXp} trên ${levelXpTarget} EXP ở cấp hiện tại`}><i style={{ width: `${xpProgress}%` }} /></div>
      <div className="player-hud-work"><span>{isDestination ? 'DESTINATION MAP' : workCareerLabel}</span><div className={`player-hud-work-actions ${isDestination ? 'is-destination' : ''}`}><button type="button" title={workCta} onClick={openWorkPanel}>{workCta} ↗</button>{!isDestination && <><button className="player-hud-fishing" type="button" title="Đi câu cá" aria-label="Đi câu cá" onClick={openFishing}>🎣 CÂU</button><button className="player-hud-home" type="button" title="My Home" aria-label="Mở My Home" onClick={openHome}>🏠 HOME</button></>}</div></div>
    </div>
